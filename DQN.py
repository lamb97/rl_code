...
class DQN:
    ...
    def __init__(self, state_dim, hidden_dim, action_dim, learning_rate, gamma, epsilon, target_update, device):
        ...
        self.epsilon = epsilon  #epslion贪婪策略
        self.target_update = target_update  #目标网络更新频率
        self.count = 0  #计数器，记录更新次数
        self.device = device
    def take_action(self,state): #eplion-贪婪策略采取动作
    ...
    def update(self, transition_dict):
        ...
        self.optimizer.zero_grad( ) # Pytorch中默认梯度会累计，这里需要将梯度显式设置为零
...
if __name__ == '__main__':
    ...
    agent = DQN(state_dim, hidden_dim, action_dim, lr, gamma, epslion, target_update, device)
        ...
        with tqdm(total=int(num_episode / 10), desc='Iteration %d' % i) as pbar:
            for i_episode in range(int(num_episode / 10)):
                ...
                if (i_episode + 1) % 10 == 0:
                    pbar.set_postfix({
                        'episode':
                            '%d' % (num_episode / 10 * i + i_episode + 1),
                        'return':
